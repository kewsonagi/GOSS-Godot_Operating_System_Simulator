...
func MoveWidget() -> void:
	...
	if(self.get_index()>0):
		RClickMenuManager.instance.AddMenuItem("Start", taskbarParent.MoveWidget.bind(self, -self.get_index()), ResourceManager.GetResource("Move"), Color.TURQUOISE)
		RClickMenuManager.instance.AddMenuItem("Left", taskbarParent.MoveWidget.bind(self, -1), ResourceManager.GetResource("Move"), Color.YELLOW_GREEN)
	...
	if(self.get_index()<get_parent().get_child_count()-1):
		RClickMenuManager.instance.AddMenuItem("Right", taskbarParent.MoveWidget.bind(self, 1), ResourceManager.GetResource("Move"), Color.GREEN_YELLOW)
		RClickMenuManager.instance.AddMenuItem("End", taskbarParent.MoveWidget.bind(self, get_parent().get_child_count()-self.get_index()), ResourceManager.GetResource("Move"), Color.MISTY_ROSE)
